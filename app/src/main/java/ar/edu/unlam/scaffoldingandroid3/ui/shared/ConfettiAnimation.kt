...
private fun generateConfettiParticles(count: Int): List<ConfettiParticle> {
    ...
    return (0 until count).map { index ->
        // Solo 1 de cada 5 será círculo
        val isRectangle = (index % 5) != 0

        ConfettiParticle(
            // 0.0 a 1.0 del ancho
            startXRatio = Random.nextFloat(),
            // Empezar bien arriba
            startY = Random.nextFloat() * 300f - 150f,
            color = colors.random(),
            size = Random.nextFloat() * 15f + 8f,
            shape = if (isRectangle) ConfettiShape.RECTANGLE else ConfettiShape.CIRCLE,
            // Mucho más movimiento
            windAmplitude = Random.nextFloat() * 100f + 50f,
            windFrequency = Random.nextFloat() * 6f + 2f,
            rotationSpeed =
                if (isRectangle) {
                    // Papelitos giran más rápido
                    Random.nextFloat() * 12f + 4f
                } else {
                    // Círculos giran más suave
                    Random.nextFloat() * 6f + 2f
                },
            // Explosión más prolongada y escalonada
...
private data class ConfettiParticle(
    // Posición X como ratio del ancho (0.0-1.0)
