...
class DeviceSensorManager @Inject constructor(
     ...
     */
    fun getAltitudeUpdates(): Flow<Double> = callbackFlow {
        if (barometer == null) {
            close(Exception("Barómetro no disponible"))
            return@callbackFlow
        }

        val barometerListener = object : SensorEventListener {
            override fun onSensorChanged(event: SensorEvent?) {
                event?.let {
     ...
     */
    private fun calculateAltitudeFromPressure(currentPressure: Float, referencePressure: Float): Double {
